...
// Mem-to-register-vector coverage through the full-reset flow.
// -----
firrtl.circuit "Mem"{
  firrtl.module public @Mem(
      in %reset: !firrtl.asyncreset [{class = "circt.FullResetAnnotation", resetType = "async"}], out %d : !firrtl.probe<vector<uint<8>, 8>>, out %d2 : !firrtl.probe<vector<uint<8>, 8>>) attributes {annotations = [
...
firrtl.circuit "Mem_Ignore" {
  firrtl.module public @Mem_Ignore(
...
firrtl.circuit  "GCTModule" {
  firrtl.module public @GCTModule(
      in %reset: !firrtl.asyncreset [{class = "circt.FullResetAnnotation", resetType = "async"}]
  ) attributes {annotations = [
    {class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}
  ]} {
    %rf_read, %rf_write = firrtl.mem Undefined {
      ...
      // CHECK:         %rf = firrtl.regreset %2, %reset, {{.*}}
  }
}

firrtl.circuit "WriteMask" {
  firrtl.module public @WriteMask(
      in %reset: !firrtl.asyncreset [{class = "circt.FullResetAnnotation", resetType = "async"}]
  ) attributes {annotations = [
    {class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}
  ]} {
...
firrtl.circuit "NLA" {
  ...
  firrtl.module public @NLA(
      in %reset: !firrtl.asyncreset [{class = "circt.FullResetAnnotation", resetType = "async"}]
  ) attributes {annotations = [
    {class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}
  ]} {
    %foo_reset = firrtl.instance foo sym @foo @Foo(in reset: !firrtl.asyncreset)
	}
}
