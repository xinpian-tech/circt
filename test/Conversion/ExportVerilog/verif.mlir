...
hw.module @WeakAndStrongSequences(in %clk: i1, in %a: i1, in %b: i1, in %c: i1, in %d: i1) {
  // CHECK: assert property (weak (a ##0 (@(posedge clk) ##1 b) ##0 c ##0 (@(posedge clk) ##1 d)));
  ...
  // CHECK: assert property (strong (b ##0 (@(posedge clk) ##1 c) ##0 (@(posedge clk) ##1 d)));
  %b0 = ltl.clocked_delay %c, posedge %clk, 1, 0 : i1
